...
public class ReviewService {
    ...
    @Autowired
    public ReviewService(ReviewRepository reviewRepository, MongoTemplate mongoTemplate, EnrollmentService enrollmentService) {
    ...
    public ReviewResponse createReview(ReviewRequest req) {
        ...
        // enforce uniqueness: repository has unique index, but check to return friendly error
        Optional<Review> existing = reviewRepository.findByCourseIdAndUserIdAndIsDeletedFalse(req.getCourseId(), req.getUserId());
    ...
    public org.springframework.data.domain.Page<ReviewResponse> listReviews(String courseId, org.springframework.data.domain.Pageable pageable) {
        org.springframework.data.domain.Page<Review> p = reviewRepository.findByCourseIdAndIsDeletedFalse(courseId, pageable);
        ...
        if (doc != null) {
            ...
            if (avg != null) {
                mongoTemplate.getCollection("courses").updateOne(new org.bson.Document("_id", new ObjectId(courseId)),
                        new org.bson.Document("$set", new org.bson.Document("avgRating", avg).append("ratingCount", count)));
            }
        }
    }
