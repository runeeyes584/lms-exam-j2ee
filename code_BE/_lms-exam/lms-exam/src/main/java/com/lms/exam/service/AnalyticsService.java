...
public class AnalyticsService {
     ...
     * Assumes orders have: createdAt (date), totalAmount (number), status = "completed"
     */
    public List<Document> getRevenueByMonth(int year) {
        ...
        log.info("Revenue aggregation for year {} returned {} results", year, results.getMappedResults().size());
    ...
    public Document getDashboardSummary() {
        ...
        long totalOrders = mongoTemplate.getCollection("orders").countDocuments(new Document("status", "completed"));
        long totalReviews = mongoTemplate.getCollection("reviews").countDocuments(new Document("isDeleted", false));
